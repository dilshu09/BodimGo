...
export default function PaymentHistory() {
    ...
    const handleExport = () => {
        ...
        const headers = ["Description", "Subtext", "Date", "Type", "Amount (LKR)", "Status", "Method"];
        const csvRows = [
            headers.join(","), // header row
            ...filteredTransactions.map(tx => [
                `"${(tx.description || '').replace(/"/g, '""')}"`,
                `"${(tx.subtext || '').replace(/"/g, '""')}"`,
                `"${tx.formattedDate || ''}"`,
                `"${tx.type || ''}"`,
                tx.amount,
                `"${tx.status || ''}"`,
        ...
        <div className="p-8 max-w-7xl mx-auto" onClick={() => setShowFilterMenu(false)}>
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2 relative">
                    ...
                    <button onClick={handleExport} className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2 font-medium">
