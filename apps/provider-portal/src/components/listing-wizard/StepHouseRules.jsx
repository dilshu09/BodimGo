...
const Toggle = ({ label, checked, onChange, subLabel }) => (
    <div className="flex items-center justify-between p-4 bg-white border border-neutral-100 rounded-2xl hover:border-primary/20 transition-all group">
        <div>
            <span className="font-semibold text-neutral-700 group-hover:text-neutral-900 transition-colors">{label}</span>
            {subLabel && <p className="text-xs text-neutral-400 mt-0.5">{subLabel}</p>}
        </div>
        <button
            onClick={() => onChange(!checked)}
            className={`w-12 h-7 flex items-center rounded-full p-1 transition-colors duration-300 ${checked ? 'bg-primary' : 'bg-neutral-200'
                }`}
        >
            <div
...
const ModernSelect = ({ label, value, onChange, options }) => (
    <div className="bg-neutral-50 p-4 rounded-xl border border-neutral-100">
        <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide mb-2 block">{label}</label>
        <select
            value={value}
...
const TimeInput = ({ label, value, onChange }) => (
    <div className="bg-neutral-50 p-4 rounded-xl border border-neutral-100">
        ...
        <input
            ...
            value={value}
            onChange={(e) => onChange(e.target.value)}
            className="w-full bg-white border-0 rounded-lg p-2 text-neutral-700 font-medium focus:ring-2 focus:ring-primary/20 shadow-sm"
        />
    </div>
);
...
const StepHouseRules = ({ data, update }) => {
    ...
    return (
        <div className="animate-fade-in space-y-6">
            <div className="mb-6">
                <h3 className="font-bold text-xl text-neutral-800">House Rules & Policies</h3>
                <p className="text-neutral-500 text-sm">Set clear expectations for your tenants.</p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 p-5">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="font-bold text-neutral-800">Additional Instructions</h4>
                </div>
                <textarea
                    className="w-full bg-neutral-50 border-0 rounded-xl p-4 text-sm focus:ring-2 focus:ring-primary/20 min-h-[100px]"
